...
def parse_config(args):
    ...
    args.no_pokestops = config.getboolean(
        'Search_Settings', 'Disable_Pokestops')
...
def get_args():
    ...
    parser.add_argument(
        '-se', '--settings', action='store_true', default=False)
    parser.add_argument(
        '-a', '--auth-service', type=str.lower,
    ...
    parser.add_argument(
        '-l', '--location', type=parse_unicode,
        help='Location, can be an address or coordinates',
        required=False)
    parser.add_argument(
        '-st', '--step-limit', help='Steps',
    ...
    parser.add_argument(
        '-m', '--mock',
        help='Mock mode. Starts the web server but '
             'not the background thread.',
        action='store_true', default=False)
    parser.add_argument(
        '-ns', '--no-server',
        help='No-Server Mode. Starts the searcher '
             'but not the Webserver.',
        action='store_true', default=False, dest='no_server')
    parser.add_argument(
        '-os', '--only-server',
        help='Server-Only Mode. Starts '
             'only the Webserver without the searcher.',
        action='store_true', default=False, dest='only_server')
    parser.add_argument(
        '-fl', '--fixed-location',
        help='Hides the search bar '
             'for use in shared maps.',
        action='store_true', default=False,
        dest='fixed_location')
    parser.add_argument(
        '-k', '--google-maps-key',
        help='Google Maps Javascript API Key',
        default=None, dest='gmaps_key')
    parser.add_argument(
        '-C', '--cors', help='Enable CORS on web server',
        action='store_true', default=False)
    parser.add_argument(
        '-D', '--db', help='Database filename',
        default='pogom.db')
    parser.add_argument(
        '-t', '--threads', help='Number of search threads',
    ...
    if args.settings:
        ...
        no_location = args.location is None
        no_username = args.username is None
        no_steps = args.step_limit is None
        if args.only_server:
            if no_location:
                parser.print_usage()
                print_missing(['-l/--location'])
                sys.exit(1)
        else:
            if no_username or no_location or no_steps:
                parser.print_usage()
                miss = ['-u/--username', '-l/--location', '-st/--step-limit']
                print_missing(miss)
                sys.exit(1)
...
def print_missing(missing_args_list):
    missing_string = ', '.join(missing_args_list)
    print '{0}: error: arguments {1} are required'.format(
        sys.argv[0], missing_string)
...
def insert_mock_data():
    ...
    locations = [l for l in
                 generate_location_steps((latitude, longitude), num_pokemon)]
...
def load_credentials(filepath):
    ...
    if not creds.get('gmaps_key'):
        raise APIKeyException(
            "No Google Maps Javascript API key entered in "
            "\config\credentials.json file! Please take a look at the wiki "
            "for instructions on how to generate this key, "
