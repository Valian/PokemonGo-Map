...
from pogom.coords import get_direction_string, get_distance
...
class Pogom(Flask):
    ...
    def list_pokemon(self):
        # todo: check if client is android/iOS/Desktop
        # for geolink, currently only supports android
        pokemon_list = []
        ...
        for pokemon in Pokemon.get_active(None, None, None, None):
            ...
            entry = {
                ...
                'card_dir': get_direction_string(origin_point, pokemon_point),
                'distance': int(get_distance(origin_point, pokemon_point)),
                'time_to_disappear': '%dm %ds' % (
                    divmod(time_to_disappear.seconds, 60)),
